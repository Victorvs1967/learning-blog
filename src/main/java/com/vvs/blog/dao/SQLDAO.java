...
import com.vvs.blog.dao.mapper.CommentMapper;
...
public final class SQLDAO {
	...
	public List<Article> listArticlesBySearchQuery(Connection c, String searchQuery, int offset, int limit) throws SQLException {
		...
		return sql.query(c, "select a.* from article a where (a.title ilike ? or a.content ilike ?) order by a.id desc limit ? offset ?", 
	...
	public int countArticlesBySearchQuery(Connection c, String searchQuery) throws SQLException {
		String q = "%" + searchQuery + "%";
		return sql.query(c, "select count(a.id) from article a where (a.title ilike ? or a.content ilike ?)", new ScalarHandler<Number>(), q, q).intValue();
	}
